...
@pytest.mark.parametrize("StorageCls", [MemoryMappedTensorStorage, FileTensorStorage])
...
def test_storage(tmpdir, length, StorageCls, data_point):
    """Tests the memory-mapped tensor storage for different nested tensor structures."""
    storage = StorageCls(tmpdir, data_point, length)
