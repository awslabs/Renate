...
by wrapping it in :code:`RenateWrapper`.

In the :code:`data_module_fn`, we load the matching tokenizer from the :code:`transformers` library.
We then use :code:`renate`'s :code:`HuggingfaceTextDataModule` to access datasets from the
`Huggingface datasets hub <https://huggingface.co/datasets>`_. This data module expects the name of
a dataset as well as a tokenizer. Here, we load the :code:`"imdb"` dataset in the first training
stage (:code:`chunk_id = 0`) and the :code:`"rotten_tomatoes"` dataset for the model update
(:code:`chunk_id = 1`).
