...
find to be the simplest and more convenient. The complete documentation is available in :py:meth:`renate.tuning.tuning.execute_tuning_job`.
...
Once completed the first step, a simple way to run a training job is to use the :code:`execute_tuning_job`,
this can work for most training needs: it can launch trainings with and without HPO,
either locally or on Amazon SageMaker.


Run a simple local training
---------------------------
...
to the following one. The arguments to be specified in the configuration are passed to the :code:`ModelUpdater`
...
* :code:`metric`: it is the target metric. Metrics measured on the validation set are prefixed with :code:`val_`,\
    while the ones measured on the training set are prefixed with :code:`train_`. Mode an metric are not really important\
    at this stage since the updater will track them but only minimize the loss (i.e., :code:`train_loss`).
* :code:`updater`: the name of the algorithm to be used for updating the model.
...
Once the training has been executed you will see some metrics printer on the screen (e.g., validation accuracy)
...
* :code:`instance_type`: the type of machine to be used for the training. The list training instances is available :ref:`here <https://aws.amazon.com/sagemaker/pricing/>`.
...
see the :ref:`Syne Tune documentation <https://github.com/awslabs/syne-tune/blob/main/docs/search_space.md>`.
If you don't know which search space to use, you can adopt a default one by calling :code:`config_space()` and passing the
