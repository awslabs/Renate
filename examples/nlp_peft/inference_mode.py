...
def main():
    ...
    with EmptyInitOnDevice(dtype=dtype, quantization_mode="llm.int8"):
        config = llama_configs["7B"]
        model = LLaMA(config=config).eval().to("cuda")
    
    inputs = torch.randint(0, 1000, size=(1, 100), dtype=torch.int, device="cuda")
